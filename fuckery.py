...
field_names = nwp_forecast_table_xarray.coords['field_name'].values

for i in range(len(LEAD_TIMES_HOURS)):
    i_other = numpy.where(
        nwp_forecast_table_xarray.coords['forecast_hour'].values
        == LEAD_TIMES_HOURS[i]
    )[0][0]
    ...
    for j in range(len(field_names)):
        ...
        # plot_in_log2_scale = field_names[j] in [nwp_model_utils.PRECIP_NAME]
        ...
        axes_object.set_xlim(
            numpy.min(nwp_forecast_table_xarray[nwp_model_utils.LONGITUDE_KEY].values),
            numpy.max(nwp_forecast_table_xarray[nwp_model_utils.LONGITUDE_KEY].values)
        )
        axes_object.set_ylim(
            numpy.min(nwp_forecast_table_xarray[nwp_model_utils.LATITUDE_KEY].values),
            numpy.max(nwp_forecast_table_xarray[nwp_model_utils.LATITUDE_KEY].values)
        )
