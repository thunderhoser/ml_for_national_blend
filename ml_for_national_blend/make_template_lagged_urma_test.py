"""Makes Chiu-net++ template with lagged URMA fields in predictors."""
...
OUTPUT_DIR_NAME = (
    ...
    'lagged_urma_template_test/templates'
)
...
METRIC_FUNCTIONS = [
    ...
    custom_losses.dual_weighted_msess(channel_weights=numpy.array([1.]), expect_ensemble=False, function_name='dwmse')
]
...
DEFAULT_OPTION_DICT = {
    ...
    chiu_net_pp_arch.INPUT_DIMENSIONS_LAGGED_TARGETS_KEY: numpy.array([432, 432, 4, 5], dtype=int),
...
def _run():
    ...
    for i in range(len(BATCH_SIZES)):
        for j in range(2):
            ...
            output_file_name = (
                '{0:s}/batch-size={1:03d}_include-lagged-targets={2:d}/'
                'model.keras'
            ).format(
                OUTPUT_DIR_NAME, BATCH_SIZES[i], int(j == 0)
            )
