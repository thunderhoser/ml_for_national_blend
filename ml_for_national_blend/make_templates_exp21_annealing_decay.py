"""Makes templates for Experiment 21 (cosine annealing & weight decay)."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment21_cosine_annealing_and_weight_decay/templates'
)
...
THIS_LIST = [list(combinations(ALL_MODEL_NAMES, r)) for r in [3]]
...
OPTIMIZER_NAMES = ['adamw', 'lion']
WEIGHT_DECAY_VALUES = numpy.array([0, 1e-5, 3e-5, 5e-5, 7e-5, 9e-5, 11e-5])
...
def _run():
    ...
    for i in range(len(OPTIMIZER_NAMES)):
        for j in range(len(WEIGHT_DECAY_VALUES)):
            model_names = MODEL_NAME_SETS[0]
            ...
            if OPTIMIZER_NAMES[i] == 'adamw':
                if WEIGHT_DECAY_VALUES[j] > 1e-10:
                    optimizer_function = keras.optimizers.AdamW(
                        gradient_accumulation_steps=10,
                        weight_decay=WEIGHT_DECAY_VALUES[j]
                    )
                    optimizer_function_string = (
                        'keras.optimizers.AdamW('
                        'gradient_accumulation_steps=10, weight_decay={0:.6f}'
                        ')'
                    ).format(WEIGHT_DECAY_VALUES[j])
                else:
                    optimizer_function = keras.optimizers.AdamW(
                        gradient_accumulation_steps=10
                    )
            ...
            output_file_name = (
                '{0:s}/optimizer={1:s}_weight-decay={2:.6f}/model.keras'
            ).format(
                OUTPUT_DIR_NAME,
                OPTIMIZER_NAMES[i],
                WEIGHT_DECAY_VALUES[j]
            )
