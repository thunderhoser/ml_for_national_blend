...
PATCH_START_ROW_HELP_STRING = (
    'Start row of patch (smaller domain).  If {0:s} == j, the first row of the '
    'patch is the [j]th row of the full NBM grid.'
).format(
    PATCH_START_ROW_ARG_NAME
)
PATCH_START_COLUMN_HELP_STRING = (
    'Start column of patch (smaller domain).  If {0:s} == k, the first column '
    'of the patch is the [k]th column of the full NBM grid.'
).format(
    PATCH_START_COLUMN_ARG_NAME
)
...
INPUT_ARG_PARSER.add_argument(
    '--' + PATCH_START_ROW_ARG_NAME, type=int, required=True,
    help=PATCH_START_ROW_HELP_STRING
)
INPUT_ARG_PARSER.add_argument(
    '--' + PATCH_START_COLUMN_ARG_NAME, type=int, required=True,
