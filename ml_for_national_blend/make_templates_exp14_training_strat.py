"""Makes templates for Experiment 14 (trng strategies for small patches)."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment14_training_strat/templates'
)
...
DEFAULT_OPTION_DICT = {
    chiu_net_pp_arch.INPUT_DIMENSIONS_CONST_KEY: numpy.array([PATCH_SIZE_ONE_DIM, PATCH_SIZE_ONE_DIM, 4], dtype=int),
    # chiu_net_pp_arch.INPUT_DIMENSIONS_2PT5KM_RES_KEY: numpy.array([PATCH_SIZE_ONE_DIM, PATCH_SIZE_ONE_DIM, 2, 22], dtype=int),
    ...
    # chiu_net_pp_arch.INPUT_DIMENSIONS_2PT5KM_RCTBIAS_KEY: numpy.array([PATCH_SIZE_ONE_DIM, PATCH_SIZE_ONE_DIM, 2, 3], dtype=int),
    ...
    chiu_net_pp_arch.PREDN_BASELINE_DIMENSIONS_KEY: numpy.array([PATCH_SIZE_ONE_DIM, PATCH_SIZE_ONE_DIM, 5], dtype=int),
    # chiu_net_pp_arch.INPUT_DIMENSIONS_LAGGED_TARGETS_KEY: numpy.array([PATCH_SIZE_ONE_DIM, PATCH_SIZE_ONE_DIM, 2, 1], dtype=int),
    ...
    chiu_net_pp_arch.NWP_POOLING_SIZE_KEY: numpy.full(MODEL_DEPTH, 2, dtype=int),
    chiu_net_pp_arch.NWP_ENCODER_NUM_CONV_LAYERS_KEY: numpy.full(MODEL_DEPTH + 1, NUM_CONV_LAYERS_PER_BLOCK, dtype=int),
    chiu_net_pp_arch.NWP_ENCODER_DROPOUT_RATES_KEY: numpy.full(MODEL_DEPTH + 1, 0.),
    ...
    chiu_net_pp_arch.DECODER_NUM_CONV_LAYERS_KEY: numpy.full(MODEL_DEPTH, NUM_CONV_LAYERS_PER_BLOCK, dtype=int),
    ...
    chiu_net_pp_arch.PREDICT_GUST_EXCESS_KEY: True,
...
# BATCH_SIZES_AXIS2 = numpy.array([1000, 2000, 3000, 4000], dtype=int)
USE_CONVNEXT_FLAGS_AXIS2 = numpy.array([0, 1], dtype=bool)
...
def _run():
    ...
    for i in range(len(GRAD_ACCUM_STEP_COUNTS_AXIS1)):
        for k in range(len(USE_CONVNEXT_FLAGS_AXIS2)):
            ...
            option_dict.update({
                ...
                chiu_net_pp_arch.USE_RESIDUAL_BLOCKS_KEY: not USE_CONVNEXT_FLAGS_AXIS2[k],
                chiu_net_pp_arch.USE_CONVNEXT_BLOCKS_KEY: USE_CONVNEXT_FLAGS_AXIS2[k]
            })
            model_object = chiu_net_pp_arch.create_model(option_dict)
    
            output_file_name = (
                '{0:s}/num-grad-accum-steps={1:03d}_use-convnext={2:d}/'
                'model.keras'
            ).format(
                OUTPUT_DIR_NAME,
                GRAD_ACCUM_STEP_COUNTS_AXIS1[i],
                int(USE_CONVNEXT_FLAGS_AXIS2[k])
            )
