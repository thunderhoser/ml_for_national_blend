...
TARGET_FIELDS_KEY = chiu_net_pp_arch.TARGET_FIELDS_KEY
...
def create_model(option_dict):
    ...
    for i in range(num_levels + 1):
        ...
        if len(these_layer_objects) == 1:
            fcst_module_layer_objects[i] = these_layer_objects[0]
        else:
            this_name = 'fcst_level{0:d}_concat'.format(i)
            fcst_module_layer_objects[i] = keras.layers.Concatenate(
                axis=-1, name=this_name
            )(these_layer_objects)
    ...
    for i in range(num_levels)[::-1]:
        ...
        for j in range(i + 1, num_levels + 1):
            this_upsamp_factor = 2 ** (j - i)

            if j == num_levels:
                this_base_name = 'dec{0:d}_up_enc{1:d}'.format(i, j)

                this_layer_object = keras.layers.UpSampling2D(
                    size=(this_upsamp_factor, this_upsamp_factor),
                    name=this_base_name
                )(fcst_module_layer_objects[j])
            else:
                this_base_name = 'dec{0:d}_up_dec{1:d}'.format(i, j)

                this_layer_object = keras.layers.UpSampling2D(
                    size=(this_upsamp_factor, this_upsamp_factor),
                    name=this_base_name
                )(decoder_layer_objects[j])

            this_layer_object = chiu_net_pp_arch.pad_layer(
                source_layer_object=this_layer_object,
                target_layer_object=fcst_module_layer_objects[i],
                padding_layer_name='{0:s}_padding'.format(this_base_name),
                num_spatiotemporal_dims=2
            )

            this_num_channels = int(numpy.round(
                0.5 * decoder_num_channels_by_level[i]
            ))

            # this_num_channels = nwp_encoder_num_channels_by_level[0]
            # if use_recent_biases:
            #     this_num_channels += rctbias_encoder_num_channels_by_level[0]
            # if num_lag_times > 0:
            #     this_num_channels += lagtgt_encoder_num_channels_by_level[0]

            this_layer_object = _get_2d_conv_block(
                ...
                regularizer_object=regularizer_object,
                do_activation=True,
                dropout_rate=upsampling_drop_rate_by_level[i],
                basic_layer_name=this_base_name
            )
        ...
        this_layer_object = _get_2d_conv_block(
            ...
            regularizer_object=regularizer_object,
            do_activation=True,
            dropout_rate=skip_drop_rate_by_level[i],
            basic_layer_name='dec{0:d}_carry_enc{0:d}'.format(i)
        )
        layer_objects_to_concat.append(this_layer_object)

        for j in range(i):
            this_base_name = 'dec{0:d}_down_enc{1:d}'.format(i, j)
            this_downsamp_factor = 2 ** (i - j)

            this_layer_object = architecture_utils.get_2d_pooling_layer(
                num_rows_in_window=this_downsamp_factor,
                num_columns_in_window=this_downsamp_factor,
                num_rows_per_stride=this_downsamp_factor,
                num_columns_per_stride=this_downsamp_factor,
                pooling_type_string=architecture_utils.MAX_POOLING_STRING,
                layer_name='{0:s}_pooling'.format(this_base_name)
            )(fcst_module_layer_objects[j])
        ...
        decoder_layer_objects[i] = chiu_net_pp_arch.create_skip_connection(
            input_layer_objects=layer_objects_to_concat,
            num_output_channels=decoder_num_channels_by_level[i],
            current_level_num=i,
            regularizer_object=regularizer_object
        )

        for k in range(num_decoder_conv_blocks_by_level[i]):
            decoder_layer_objects[i] = _get_2d_conv_block(
                input_layer_object=decoder_layer_objects[i],
                do_convnext_v2=do_convnext_v2,
                use_spectral_norm=use_spectral_norm,
                num_conv_layers=1,
                filter_size_px=7,
                ...
                do_activation=True,
                dropout_rate=skip_drop_rate_by_level[i],
                basic_layer_name='dec{0:d}_block{1:d}'.format(i, k)
            )

    if include_penultimate_conv:
        decoder_layer_objects[0] = _get_2d_conv_block(
            input_layer_object=decoder_layer_objects[0],
            do_convnext_v2=do_convnext_v2,
            use_spectral_norm=use_spectral_norm,
            num_conv_layers=1,
            filter_size_px=7,
