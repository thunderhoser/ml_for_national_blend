"""Creates Chiu-net++ templates for Experiment 8.

Experiment 8 builds on the models from Experiment 7 -- with more NWP models,
more NWP variables, more NWP lead times, and more URMA lag times in the
predictors.  Experiment 8 also uses residual layers, unlike Experiment 7.
"""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment08_more_predictors/templates'
)
...
METRIC_FUNCTIONS = [
    custom_metrics.max_prediction(channel_index=0, temperature_index=0, u_wind_index=-1, v_wind_index=-1, dewpoint_index=-1, gust_index=-1, expect_ensemble=False, function_name='temp_max_prediction_celsius'),
    custom_metrics.min_prediction(channel_index=0, temperature_index=0, u_wind_index=-1, v_wind_index=-1, dewpoint_index=-1, gust_index=-1, expect_ensemble=False, function_name='temp_min_prediction_celsius'),
    custom_metrics.spatial_max_bias(channel_index=0, temperature_index=0, u_wind_index=-1, v_wind_index=-1, dewpoint_index=-1, gust_index=-1, expect_ensemble=False, function_name='temp_spatial_max_bias'),
    custom_metrics.spatial_min_bias(channel_index=0, temperature_index=0, u_wind_index=-1, v_wind_index=-1, dewpoint_index=-1, gust_index=-1, expect_ensemble=False, function_name='temp_spatial_min_bias'),
    custom_metrics.mean_squared_error(channel_index=0, temperature_index=0, u_wind_index=-1, v_wind_index=-1, dewpoint_index=-1, gust_index=-1, expect_ensemble=False, function_name='temp_mse_celsius2'),
...
# TODO(thunderhoser): Miniminibatch size will be 1.
...
# TODO(thunderhoser): Subject to change, based on results of Experiment 7.
BATCH_SIZE = 25
L2_WEIGHT = 1e-7

# BATCH_SIZES = numpy.array([25, 50, 75, 100], dtype=int)
...
DEFAULT_OPTION_DICT = {
    ...
    chiu_net_pp_arch.L2_WEIGHT_KEY: L2_WEIGHT,
...
NWP_LEAD_TIME_COUNTS_AXIS1 = numpy.array([1, 2, 3, 4, 6], dtype=int)
URMA_LAG_TIME_COUNTS_AXIS2 = numpy.array([1, 2, 3, 5, 7], dtype=int)
NWP_VARIABLE_COUNTS_AXIS3 = numpy.array([7, 12, 16, 20], dtype=int)
...
def _run():
    ...
    for i in range(len(NWP_LEAD_TIME_COUNTS_AXIS1)):
        for j in range(len(URMA_LAG_TIME_COUNTS_AXIS2)):
            for k in range(len(NWP_VARIABLE_COUNTS_AXIS3)):
                input_dims_2pt5km = numpy.array([
                    432, 432,
                    NWP_LEAD_TIME_COUNTS_AXIS1[i],
                    3 * NWP_VARIABLE_COUNTS_AXIS3[k] + 1
                ], dtype=int)

                input_dims_10km = numpy.array([
                    108, 108,
                    NWP_LEAD_TIME_COUNTS_AXIS1[i],
                    NWP_VARIABLE_COUNTS_AXIS3[k]
                ], dtype=int)
                ...
                output_file_name = (
                    '{0:s}/num-nwp-lead-times={1:d}_num-urma-lag-times={2:d}_'
                    'num-nwp-variables={3:02d}/model.keras'
                ).format(
                    OUTPUT_DIR_NAME,
                    NWP_LEAD_TIME_COUNTS_AXIS1[i],
                    URMA_LAG_TIME_COUNTS_AXIS2[j],
