"""Creates Chiu-net++ templates for Experiment 7.

This is the first test of a lot of new bells and whistles, like the missing-data
policy and comparing to raw NWP forecasts in the loss function.
"""
    ...
    for i in range(len(BATCH_SIZES)):
        for j in range(len(L2_WEIGHTS)):
            ...
            option_dict.update({
                ...
                chiu_net_pp_arch.OPTIMIZER_FUNCTION_KEY: optimizer_function,
                chiu_net_pp_arch.L2_WEIGHT_KEY: L2_WEIGHTS[j]
            })
            model_object = chiu_net_pp_arch.create_model(option_dict)

            output_file_name = (
                '{0:s}/batch-size={1:03d}_l2-weight={2:.10f}/model.keras'
            ).format(
                OUTPUT_DIR_NAME, BATCH_SIZES[i], L2_WEIGHTS[j]
            )
